...
export async function sendAccountDeletionEmail(toEmail: string, code: string) {
  const transporter = nodemailer.createTransport({
    host: "smtp.gmail.com",
    port: 465,
    secure: true,
  ...
  await transporter.sendMail({
    from: `"NEXT Notes" <${process.env.SMTP_EMAIL}>`,
    to: toEmail,
    subject: "Confirm Account Deletion — NEXT Notes",
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #040209; color: #F4F2F7; max-width: 480px; margin: 0 auto; padding: 40px 24px; border-radius: 16px;">
        <div style="text-align: center; margin-bottom: 32px;">
        ...
        <p style="color: rgba(244,242,247,0.7); font-size: 14px; line-height: 1.6; margin-bottom: 8px;">
          We received a request to <strong style="color: #F87171;">permanently delete</strong> your NEXT Notes account and all associated data — notes, vault, and tasks.
        </p>
        <p style="color: rgba(244,242,247,0.7); font-size: 14px; line-height: 1.6; margin-bottom: 24px;">
          Enter the code below to confirm. It expires in <strong style="color: #F4F2F7;">60 seconds</strong>.
        </p>

        <div style="background: rgba(239,68,68,0.10); border: 1px solid rgba(239,68,68,0.25); border-radius: 12px; padding: 24px; text-align: center; margin-bottom: 24px;">
          <p style="margin: 0 0 8px; font-size: 12px; color: rgba(244,242,247,0.4); letter-spacing: 2px; text-transform: uppercase;">Deletion Code</p>
          <p style="margin: 0; font-size: 36px; font-weight: 800; letter-spacing: 8px; color: #F87171; font-family: 'Courier New', monospace;">${code}</p>
        </div>

        <p style="color: rgba(244,242,247,0.4); font-size: 12px; line-height: 1.6; text-align: center;">
          If you didn't request this, ignore this email — your account is safe.<br/>
          This action is <strong style="color: #F87171;">irreversible</strong> once confirmed.
        </p>
      </div>
    `,
  });
